...
class FlutterPrinterSdkPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private fun findUsbPathByVidPid(call: MethodCall, result: Result) {
        val vendorId = call.argument<Int>("vendorId") ?: return result.error(
            "MissingVendorId",
            "Vendor ID is missing",
            null
        )
        val productId = call.argument<Int>("productId") ?: return result.error(
            "MissingProductId",
            "Product ID is missing",
            null
        )
        val name =
            call.argument<String>("name") ?: return result.error("name", "name  is missing", null)

        // Tìm thiết bị USB dựa trên VID và PID
        val usbDevices = POSConnect.getUsbDevice(POSConnect.getAppCtx())
        val device =
            usbDevices.find { it.productName == name && it.vendorId == vendorId && it.productId == productId }
        ...
        data?.forEach { command ->
            ...
            when (method) {
                ...
                "printText" -> {
                    ...
                    val textSize = params?.get("textSize") as? Int
    ...
    private fun checkPermissions(result: Result) {
        activity?.let {
            ...
            result.error(
                ...
                "Cannot check permissions without activity",
                null
            )
        }
    }
